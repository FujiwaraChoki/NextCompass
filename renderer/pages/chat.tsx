...
const Chat = () => {
    ...
    const getChatGPTReply = async (prompt: string) => {
        // Implement
    };
    ...
    const sendMessage = async () => {
        ...
        if (tab === 0) {
            ...
            setChatGPTMessages(messages => [...messages, {
                message: response,
                type: 'bot',
            }]);
        } else if (tab === 1) {
            ...
            const response = await fetch(`https://api.betterapi.net/youdotcom/chat?message=${message}&key=LJ3TGSI1X6N476W9Q30JYRV31R5U1XPHGBA`, {
                "method": "GET",
                "headers": {
            ...
            setBingMessages(messages => [...messages, {
                message: message,
                type: 'user',
            }]);
    ...
    return (
        <div className="rounded-g border bg-base-300 m-20">
            ...
            <div className="justify-center px-4 py-16 bg-base-200 relative">
                <h1 className="text-4xl font-bold m-5">{tabNames[tab]}</h1>

                <div className="flex-1 flex-grow overflow-y-auto px-4 py-2">
